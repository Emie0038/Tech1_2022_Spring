...
SELECT * FROM tbook WHERE nPublishingYear < 2000
...
SELECT * FROM tloan WHERE cCpr=0305393207 AND YEAR (dLoan) = 2014;
...
SELECT cName, cSurname, COUNT(t.nAuthorID) AS 'Books' FROM tauthor
LEFT JOIN tauthorship t ON tauthor.nAuthorID = t.nAuthorID
GROUP BY tauthor.nAuthorID
ORDER BY COUNT(tauthor.nAuthorID) DESC;
...
LEFT JOIN ttheme t2 ON t.nThemeID = t2.nThemeID WHERE nPublishingCompanyID = 32;

#11#
SELECT tauthor.cName, tauthor.cSurname, MIN(tbook.nPublishingYear)
FROM tauthor
LEFT JOIN tbook on nBookID= nBookID
LEFT JOIN tauthorship t on tauthor.nAuthorID = t.nAuthorID
WHERE nBookID IS NOT NULL
...
AND tbooktheme.nBookID = tbook.nBookID ORDER BY cName;
...
FROM tmember, tauthor WHERE dNewMember LIKE '2013-12%' AND tauthor.cName = 'William';
...
WHERE dNewMember = (SELECT MIN(tmember.dNewMember) FROM tmember);
...
SELECT nPublishingYear,  COUNT(*) AS 'Number of books published', cName
...
SELECT tbook.cTitle, tbook.npublishingyear AS 'Year', nPublishingCompanyID AS 'ID' FROM tbook
...
SELECT cName, cSurname FROM tmember WHERE dNewMember > 2016 AND cAddress IS NULL;
...
WHERE  tbook.cTitle LIKE 'The Tale%' AND cName != 'Lynch Inc'
ORDER BY cTitle;
...
SELECT COUNT(*) AS 'No. of Members who loaned 2013' FROM tmember
...
SELECT tbook.cTitle AS 'Title', COUNT(*) AS 'No. Of Authors'
FROM tbook
